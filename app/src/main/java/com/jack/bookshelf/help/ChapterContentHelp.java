...
//    private Map<String, Pattern> adMap = new HashMap<>();
    private final Map<String, String> adMap = new HashMap<>();
    // 缓存长表达式，使用普通方式替换
    private final Map<String, String> adMapL = new HashMap<>();
...
//            rule = Pattern.compile("((" + buffer + ")(\\p{P}{0,2})){1,10}(" + buffer + ")");
            rule = ("((" + buffer + ")(\\p{P}{0,2})){1,20}(" + buffer + ")((\\p{P}{0,12})(?=\\p{P}{2}))?");
...
//        Pattern p=Pattern.compile(rule);
    ...
    private final Map<String, Pattern> AdPatternMap = new HashMap<>();
    // 不包含符号的文本形式的规则缓存。用于广告规则的第二次替换，以解决如下问题： 规则有 abc def，而实际出现了adefbc
    private final Map<String, String> AdStringDict = new HashMap<>();
...
*/
    ...
    //  段落换行符
    private static final String SPACE_BEFORE_PARAGRAPH = "\n    ";
    //  段落末位的标点
    private static final String MARK_SENTENCES = "？。！?!~”\"";
    ...
    private static final String MARK_SENTENCES_F = "啊嘛吧吗噢哦了呢呐";
    ...
    private static final String PARAGRAPH_DIAGLOG = "^[\"”“][^\"”“]+[\"”“]$";
    //  限制字典的长度
